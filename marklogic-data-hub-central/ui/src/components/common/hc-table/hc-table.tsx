...
function HCTable({className, rowStyle, childrenIndent, data, keyUtil, expandedRowKeys, nestedParams, pagination, rowClassName, baseIndent = 0, showHeader = true, showExpandIndicator = false, onExpand, expandedRowRender, ...props}: Props): JSX.Element {
  const expandConfig = {
  ...
  if (childrenIndent && !expandedRowRender) {
    ...
    expandConfig.renderer = (row, rowIndex) => renderNested({row, data, keyUtil, baseIndent, showHeader, indentList, rowIndex, expandIndicatorStyle: showExpandIndicator, selectRow, ...nestedParams});
  }
...
const renderRow = ({row, rowIndex, parentRowIndex, keyUtil, indentList, baseIndent, headerColumns, showHeader, iconCellList, state, showIndicator, isExpanded, bordered, selectRow}) => {
  const [expandedNestedRows] = state;
  const nextColumnHasStaticWidth = headerColumns[0].width && !`${headerColumns[0].width}`.includes("%");
  //let [selectedNestedRows] = selectRow.
  ...
  return <div key={expandKey} className={`${styles.childrenIndentTableRow} hc-table_row`} data-row-key={expandKey}>
    ...
    {selectRow ? <div style={{...selectRow.selectColumnStyle({record: row})}} className={styles.childrenIndentSelectCell}>{selectRow.selectionRenderer({disabled: false, record: row, childrenIndent: true})}</div> : null}
...
const renderNested = ({row, parentRowIndex, keyUtil, baseIndent, indentList, headerColumns, showHeader, iconCellList, state, expandIndicatorStyle, selectRow}) => {
  ...
  if (row.children) {
    ...
    while (childrenList.length > 0) {
      ...
      let tableRow = renderRow({row: currentRow, rowIndex, parentRowIndex, keyUtil, baseIndent, indentList, headerColumns, showHeader, iconCellList, state, showIndicator: currentRow.children, isExpanded, bordered, selectRow});
